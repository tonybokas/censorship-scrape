...
txt_file = Path(str(file_name) + str('.txt')) # creates a TXT file.
...
print('\nExecuted:', time_now.strftime('%H:%M:%S on %A, %B %d, %Y'))
print('\nMethod: "Keyword Match Compiler" Python script by Antonios J. Bokas')

print('\n--------------------------------------------------')
print('SECTION 1. PARSED HTML FILES')
...
try:
	...
	for file_name in os.listdir(directory): # determines which directory will be processed.
		...
		if file_name.endswith('.html'): # determines that only HTML files will be processed.
			...
			for item in keyword_4_match_list:
			...
			for item in keyword_5_match_list:
			...
			for item in keyword_6_match_list:
